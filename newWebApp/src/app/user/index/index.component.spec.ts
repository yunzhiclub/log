...
describe('IndexComponent', () => {
  ...
  it('当前页、总页数、每页大小', () => {
    ...
    /* 断言绑定了C层的分页值 */
    expect(text).toContain(`第${component.params.page + 1}/${component.pageUser.totalPages}页`);
    expect(text).toContain(`第${component.params.page + 1}/${component.pageUser.totalPages}页 每页${component.params.size}条`);
  });
  ...
  it('onPage 功能测试', () => {
    ...
    /* 越界测试：期望不改变当前页码值，loadData仅被前面的代码调用了1次（本次未调用）*/
    component.pageUser.totalPages = 5;
    component.onPage(5);
    expect(component.params.page).toEqual(3);
    expect(component.loadData).toHaveBeenCalledTimes(1);
  });
  ...
  it('下一页(尾页）样式', () => {
    ...
    cssSelector.forEach((value) => {
      ...
      component.pageUser.totalPages = 4;
      fixture.detectChanges();
      ...
      /* 总页数为3页，下一页（尾页)不可点击 */
      component.pageUser.totalPages = 3;
      fixture.detectChanges();
  ...
  it('点击尾页', () => {
    ...
    spyOn(component, 'onPage');
    liElement.click();
    expect(component.onPage).toHaveBeenCalledWith(component.pageUser.totalPages - 1);
  });
